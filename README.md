...
- **题库**：271 题，按 1-5 星重要性分级，支持「必刷/拓展」双开关筛选；新增 **4 个主题库（4 级压缩）**——库1 完整(271) / 库2 标准(236) / 库3 精简(199) / 库4 极简(99)，首次打开可选，编辑模式可切换
- **四模式**：🎴 抽背卡(测试) + 📖 记背知识(学习) + 🎯 面试前指导(突击) + ✏️ 编辑(CRUD+存档回滚+切换题库)
- **双形态**：单文件 `面试背记学习卡v2.10.html`（双击即用）+ `dist/`（PWA 分片可部署）
- **云端同步**：可选接入 Supabase，昵称登录后学习进度/设置/编辑题库自动上云，换设备不丢；多用户数据完全隔离（v2.10 强化：旧本地数据自动迁移、push 失败自动重试、登录冲突自动兜底）
...
直接双击打开 `dist/面试背记学习卡v2.10.html` 即可，无需联网、无需服务器。数据已内联，`file://` 协议下功能完整可用。
