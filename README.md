...
- **双形态**：单文件 `面试背记学习卡v2.11.html`（双击即用）+ `dist/web/`（PWA 分片可部署）
- **云端同步**：可选接入 Supabase，昵称登录后学习进度/设置/编辑题库自动上云，换设备不丢；多用户数据完全隔离（v2.10 强化：旧本地数据自动迁移、push 失败自动重试、登录冲突自动兜底；**v2.11 强化**：confirmLogin 用 sync push 等推送完成再 reload，编辑工具栏新增「☁️ 从云端拉取」「🔍 诊断」两个应急按钮，题库空白可一键恢复）
...
直接双击打开 `dist/面试背记学习卡v2.11.html` 即可，无需联网、无需服务器。数据已内联，`file://` 协议下功能完整可用。
...
产出：
- `dist/面试背记学习卡v2.10.html` —— 单文件版（数据内联）
