...
- **双形态**：单文件 `面试背记学习卡v2.12.html`（双击即用）+ `dist/web/`（PWA 分片可部署）
- **云端同步**：可选接入 Supabase，昵称登录后学习进度/设置/编辑题库自动上云，换设备不丢；多用户数据完全隔离（v2.10 强化：旧本地数据自动迁移、push 失败自动重试、登录冲突自动兜底；**v2.11 强化**：confirmLogin 用 sync push 等推送完成再 reload，编辑工具栏新增「☁️ 从云端拉取」「🔍 诊断」两个应急按钮，题库空白可一键恢复；**v2.12 扩充**：题库 271→361 题，新增 MCP / LangGraph / Agent 智能体 / AI 应用工程 / 提示词工程 / SQLAlchemy / AI 场景设计 共 7 个分类，档位上限适当放宽（库3<320 / 库4<180））
...
直接双击打开 `dist/面试背记学习卡v2.12.html` 即可，无需联网、无需服务器。数据已内联，`file://` 协议下功能完整可用。
...
- **异常自救**：进入「编辑」模式 → 点工具栏「🔍 诊断」一键 dump 状态到 Console 截图给我；或点「☁️ 从云端拉取」把云端最新数据覆盖本地
...
| `NG` | Nginx | Nginx |
| `HTTP` | HTTP协议 | HTTP协议 |
| `LLM` | 大模型应用 | 大语言模型、RAG、LangChain（低代码/微调/部署/预训练/Agent） |
| `LG` | LangGraph | LangGraph |
| `AG` | Agent 智能体 | Agent 智能体 |
| `AE` | AI 应用工程 | AI 应用工程 |
| `PE` | 提示词工程 | 提示词工程 |
| `MCP` | MCP | MCP（模型上下文协议） |
...
产出：
- `dist/面试背记学习卡v2.12.html` —— 单文件版（数据内联）
